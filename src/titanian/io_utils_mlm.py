...
class Collater_finetune(SimpleCollater):
    ...
    def _prep(self, sequences, mhc1, mhc2, labels):
        seq_ = list(sequences[:])
        seq = []
        label = list(labels[:])
        mhc1_ = list(mhc1[:])
        ...
        return seq, m1, m2, label

    def __call__(self, batch):
        sequences, mhc1, mhc2, fractions = zip(*batch)
        return self._prep(sequences, mhc1, mhc2, fractions)
...
class CSVDataset_inf_mhc(Dataset):
    def __init__(self, data: pd.DataFrame):
        self.seq = data["peptide"]
        self.mhc = data["pseudo"]
        self.label = data["label"]
...
class Collater_inf_mhc(SimpleCollater):
    def __init__(
        self,
        alphabet: str,
        pad=False,
    ...
    def __call__(self, batch):
        sequences, m1, m2, label = zip(*batch)
        return self._prep(sequences, m1, m2, label)
...
class TaskSpecificBatchSampler_sampling(Sampler):
    ...
    def _sample_indices_by_task(self):
        ...
        for task, indices in task_indices.items():
            fraction = self.task_fraction_dict.get(
                task, 1
            )  # Default fraction is 1 if not specified
            sample_size = int(len(indices) * fraction)
            sampled_indices[task] = np.random.choice(
    ...
    def __len__(self):
        ...
        total_sample_size = sum(
            len(indices) for indices in self.sampled_indices.values()
        )
...
class Collater_merged(SimpleCollater):
    ...
    def _prep_task4(self, sequences, mhc, task):
        tgt_ = list(sequences[:])
        tgt = []
        src = []
        mask = []
    ...
    def _prep_task5(self, sequences, mhc, labels, task):
        seq_ = list(sequences[:])
        seq = []
        label = list(labels[:])
        mhc2_ = list(mhc[:])
    ...
    def _prep_task6(self, sequences, mhc, task):
        tgt_ = list(sequences[:])
        tgt = []
        src = []
        mask = []
    ...
    def _prep_task7(self, sequences, tcr, labels, task):
        seq_ = list(sequences[:])
        seq = []
        label = list(labels[:])
        tcr2_ = list(tcr[:])
    ...
    def _prep_task8(self, sequences, mhc, labels, task):
        seq_ = list(sequences[:])
        seq = []
        label = list(labels[:])
        mhc1_ = list(mhc[:])
    ...
    def __call__(self, batch):
        ...
        if task[0] in self.task_list:
            if task[0] == 1:
                ...
                return self._prep_task4(peptide, pseudo1, task)
            elif task[0] == 5:
                return self._prep_task5(peptide, pseudo2, label, task)
            elif task[0] == 6:
                return self._prep_task6(peptide, pseudo2, task)
            elif task[0] == 7:
                return self._prep_task7(peptide, CDR3b, label, task)
            elif task[0] == 8:
                return self._prep_task8(peptide, pseudo1, label, task)
        else:
            pass
