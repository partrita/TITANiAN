...
class MaskedConv2d(nn.Conv2d):
    ...
    def forward(self, x, input_mask=None):
        ...
        return (
            super()
            .forward(x.permute(0, 3, 1, 2).contiguous())
            .permute(0, 2, 3, 1)
            .contiguous()
        )
...
class MaskedCausalConv1d(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size=1, dilation=1, groups=1, init=None
    ):
        """
    ...
    def _init_recurrent_state(self, batch):
        ...
        self.recurrent_state = torch.zeros(
            batch, self.in_channels, self.zeros, device=self.bias.device
        )
    ...
    def _generate(self, x_i):
        ...
        activations = F.conv1d(
            f, self.weight, self.bias, dilation=self.dilation, groups=self.groups
        )
...
class ConditionalAttention(nn.Module):
    ...
    def forward(self, embeddings, condition, mask_condition=False):
        ...
        )  # Unsqueeze to make it (batch_size, 1, hidden_dim)
        ...
        )  # Normalize over the sequence length
        ...
        augmented_embeddings = torch.cat(
            [embeddings, context.repeat(1, embeddings.shape[1], 1)], dim=-1
        )
...
class Sharedencoder(nn.Module):
    def __init__(
        ...
        self.fc_layers = nn.ModuleList(
            [nn.Linear(dim, d_model * 20) for dim in tensor_dims]
        )

    def forward(self, antigen, pseudo1, pseudo2, tcr, task):
        ...
        if task[0] in [3, 4, 8]:
            ...
            projected_tensor = [
                fc(tensor) for fc, tensor in zip(self.fc_layers, [antigen, mhc_1])
            ]
            projected_tensor = [
                tensor.view(-1, 20, self.d_model) for tensor in projected_tensor
            ]
            ...
            projected_tensor = [
                fc(tensor) for fc, tensor in zip(self.fc_layers, [antigen, mhc_2])
            ]
            projected_tensor = [
                tensor.view(-1, 20, self.d_model) for tensor in projected_tensor
            ]
...
class task1_encoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.embedder = ConditionedByteNetDecoder(
            n_tokens,
            d_embedding,
            d_model,
            n_layers,
...
class task1(nn.Module):
    def __init__(
        ...
        self.task1_encoder = task1_encoder(
            n_tokens=n_tokens,
            d_embedding=d_embedding,
            d_model=d_model,
            n_layers=n_layers,
...
class task2345678_decoder(nn.Module):
...
class task2(nn.Module):
    def __init__(
        ...
        self.task2_encoder = task2_encoder(
            n_tokens=n_tokens,
            d_embedding=d_embedding,
            d_model=d_model,
            n_layers=n_layers,
        ...
        self.task2_decoder = task2345678_decoder(d_model=d_model)
...
class task3(nn.Module):
    def __init__(
        ...
        self.task3_encoder = task34568_encoder(
            n_tokens=n_tokens,
            d_embedding=d_embedding,
            d_model=d_model,
            n_layers=n_layers,
        ...
        self.task3_decoder = task2345678_decoder(d_model=d_model)

    def forward(self, p, m1, m2, tcr, task):
        shared_e = self.shared_encoder(p, m1, m2, tcr, task)
...
class task4(nn.Module):
    def __init__(
        ...
        self.task4_encoder = task34568_encoder(
            n_tokens=n_tokens,
            d_embedding=d_embedding,
            d_model=d_model,
            n_layers=n_layers,
        ...
        self.task4_decoder = task2345678_decoder(d_model=d_model)
...
class task5(nn.Module):
    def __init__(
        ...
        self.task5_encoder = task34568_encoder(
            n_tokens=n_tokens,
            d_embedding=d_embedding,
            d_model=d_model,
            n_layers=n_layers,
        ...
        self.task5_decoder = task2345678_decoder(d_model=d_model)
...
class task6(nn.Module):
    def __init__(
        ...
        self.task6_encoder = task34568_encoder(
            n_tokens=n_tokens,
            d_embedding=d_embedding,
            d_model=d_model,
            n_layers=n_layers,
        ...
        self.task6_decoder = task2345678_decoder(d_model=d_model)
...
class task7_encoder(nn.Module):
    ...
    def forward(self, antigen, tcr_2):
        ...
        projected_tensor = [
            fc(tensor) for fc, tensor in zip(self.fc_layers, [antigen, tcr_2])
        ]
        projected_tensor = [
            tensor.view(-1, 20, self.d_model) for tensor in projected_tensor
        ]
...
class task7(nn.Module):
    def __init__(
        ...
        self.task7_encoder = task7_encoder(
            n_tokens=n_tokens,
            d_embedding=d_embedding,
            d_model=d_model,
            n_layers=n_layers,
        ...
        self.task7_decoder = task2345678_decoder(d_model=d_model)
...
class task8(nn.Module):
    def __init__(
        ...
        self.task8_encoder = task34568_encoder(
            n_tokens=n_tokens,
            d_embedding=d_embedding,
            d_model=d_model,
            n_layers=n_layers,
        ...
        self.task8_decoder = task2345678_decoder(d_model=d_model)

    def forward(self, p, m1):
        shared_e = self.shared_encoder(p)
        task8_e = self.task8_encoder(p, m1)
        output = self.task8_decoder(shared_e, task8_e)
        return shared_e, task8_e, output
...
class Merged_model(nn.Module):
    def __init__(
        ...
        self.discriminator = Discriminator(
            input_size=d_model * 20, hidden_size=d_model * 5, output_size=8
        )
    ...
    def forward(self, p, task, frac=[0.0], m1=PAD * 50, m2=PAD * 50, t2=PAD * 25):
        ...
        output_4 = self.task4_decoder(shared_e, task3_e)
        output_5 = self.task5_decoder(shared_e, task5_e)
        output_6 = self.task6_decoder(shared_e, task5_e)
        ...
        results = [
            (shared_e, task1_e, task2_e, task3_e, task3_e, task5_e, task5_e, task7_e),
