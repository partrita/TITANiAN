...
def calculate_precision_recall(y_true, y_pred):
    print("unique values in y_pred", torch.unique(y_pred).shape[0])
    if torch.unique(y_pred).shape[0] == 1:
        print("This model is not trained yet")
        return 0, 0, 0
    precision, recall, threshold = precision_recall_curve(
        y_true.squeeze().detach().cpu().numpy(), y_pred.squeeze().detach().cpu().numpy()
    )
    auprc = auc(recall, precision)
    precision = precision_score(
        y_true.detach().cpu().numpy(), (y_pred > 0.5).float().detach().cpu().numpy()
    )
    recall = recall_score(
        y_true.detach().cpu().numpy(), (y_pred > 0.5).float().detach().cpu().numpy()
    )
...
def calculate_f1_score(y_true, y_pred):
    y_pred_class = (y_pred > 0.5).float()
    f1 = f1_score(y_true.detach().cpu().numpy(), y_pred_class.detach().cpu().numpy())
    return f1
...
def evaluate_class_final(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for seq, m1, m2, label, task in dataloader:
            seq = seq.to(device)
            m1, m2 = m1.to(device), m2.to(device)
            label = torch.FloatTensor(label).unsqueeze(-1).to(device)
            _, output = model(seq, m1, m2, tcr=[0], task=task)
    ...
    acc = calculate_accuracy(all_targets, all_preds)
    auroc = calculate_auroc(all_targets, all_preds)
    precision, recall, auprc = calculate_precision_recall(all_targets, all_preds)
    f1_score = calculate_f1_score(all_targets, all_preds)

    return total_loss / len(dataloader), acc, auroc, precision, recall, auprc, f1_score


def evaluate_merged(model, dataloader, device, task_list=[1, 2, 3, 4, 5, 6, 7, 8, 9]):
    model.eval()
    total_loss = 0.0
    ...
    total_masked_predictions_5 = 0
    all_preds_task6 = []
    all_targets_task6 = []
    all_preds_task7 = []
    all_targets_task7 = []
    correct_predictions_8 = 0
    total_masked_predictions_8 = 0
    all_preds_task9 = []
    all_targets_task9 = []
        ...
        for batch_idx, (tuple_, task) in enumerate(dataloader):
            if task[0] == 1:
                ...
                correct_predictions_1 += (
                    (masked_predicted == masked_targets).sum().item()
                )
            ...
            if task[0] == 5:
                ...
                correct_predictions_5 += (
                    (masked_predicted == masked_targets).sum().item()
                )
                total_masked_predictions_5 += masked_targets.numel()
            if task[0] == 6:
                src, m2, label = tuple_
                src = src.to(device)
                m2 = m2.to(device)
            ...
            if task[0] == 8:
                ...
                correct_predictions_8 += (
                    (masked_predicted == masked_targets).sum().item()
                )
                total_masked_predictions_8 += masked_targets.numel()
            if task[0] == 9:
                src, t2, label = tuple_
                src = src.to(device)
                t2 = t2.to(device)
                ...
                output = outputs[8]
                all_preds_task9.extend(output.detach().cpu())
                all_targets_task9.extend(label.detach().cpu())
    ...
    if 2 in task_list:
        ...
        precision_task2, recall_task2, auprc_task2 = calculate_precision_recall(
            all_targets_task2, all_preds_task2
        )
        f1_score_task2 = calculate_f1_score(all_targets_task2, all_preds_task2)
    else:
        acc_task2 = 0
        auroc_task2 = 0
        precision_task2 = 0
    ...
    if 3 in task_list:
        ...
        precision_task3, recall_task3, auprc_task3 = calculate_precision_recall(
            all_targets_task3, all_preds_task3
        )
        f1_score_task3 = calculate_f1_score(all_targets_task3, all_preds_task3)
    else:
        acc_task3 = 0
        auroc_task3 = 0
        precision_task3 = 0
        ...
        f1_score_task3 = 0
    if 4 in task_list:
        all_preds_task4 = torch.stack(all_preds_task4)
        ...
        precision_task4, recall_task4, auprc_task4 = calculate_precision_recall(
            all_targets_task4, all_preds_task4
        )
        f1_score_task4 = calculate_f1_score(all_targets_task4, all_preds_task4)
    else:
        acc_task4 = 0
        auroc_task4 = 0
        precision_task4 = 0
        ...
        f1_score_task4 = 0
    if 5 in task_list:
        accuracy_5 = correct_predictions_5 / total_masked_predictions_5
    else:
        accuracy_5 = 0
    if 6 in task_list:
        all_preds_task6 = torch.stack(all_preds_task6)
        ...
        precision_task6, recall_task6, auprc_task6 = calculate_precision_recall(
            all_targets_task6, all_preds_task6
        )
        f1_score_task6 = calculate_f1_score(all_targets_task6, all_preds_task6)
    else:
        acc_task6 = 0
        auroc_task6 = 0
        precision_task6 = 0
    ...
    if 7 in task_list:
        ...
        precision_task7, recall_task7, auprc_task7 = calculate_precision_recall(
            all_targets_task7, all_preds_task7
        )
        f1_score_task7 = calculate_f1_score(all_targets_task7, all_preds_task7)
    else:
        acc_task7 = 0
        auroc_task7 = 0
        precision_task7 = 0
        ...
        f1_score_task7 = 0
    if 8 in task_list:
        accuracy_8 = correct_predictions_8 / total_masked_predictions_8
    else:
        accuracy_8 = 0
    if 9 in task_list:
        all_preds_task9 = torch.stack(all_preds_task9)
        all_targets_task9 = torch.stack(all_targets_task9)
        acc_task9 = calculate_accuracy(all_targets_task9, all_preds_task9)
        auroc_task9 = calculate_auroc(all_targets_task9, all_preds_task9)
        precision_task9, recall_task9, auprc_task9 = calculate_precision_recall(
            all_targets_task9, all_preds_task9
        )
        f1_score_task9 = calculate_f1_score(all_targets_task9, all_preds_task9)
...
def calculate_ppvn_top_n(predicted_probs, actuals):
    ...
    true_count = sum(1 for x in actuals if x == 1)
